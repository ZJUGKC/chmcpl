...
int chmc_add_tree(struct chmcFile *chm, const char *dir)
{
	struct dir_tree_global dtg;
	char cwd[PATH_MAX], _path[PATH_MAX];
	int err;
	...
	if (getcwd(cwd, PATH_MAX) == NULL)
		chmcerr_return_msg("getcwd: %s", strerror(errno));

	strcpy(dtg.path, "/");
	if (dir != NULL)
		strncpy(dtg.prefix, dir, PATH_MAX);
	else
		strncpy(dtg.prefix, cwd, PATH_MAX);

	path_add_slash(dtg.prefix);
	dtg.depth = 0;
	dtg.cb_dir = _add_tree_dir;
	dtg.cb_file = _add_tree_file;
	dtg.user = chm;
	dtg._path = _path;

	err = dir_tree_walk(&dtg, dtg.prefix);
	if (err)
		chmcerr_set(err, "entry: %s:", _path);
                         ...
                         struct dir_tree_local *dtl)
{
	...
	strncat(dtg->_path, dtg->path, PATH_MAX);
	strncat(dtg->_path, dtl->entry->d_name, PATH_MAX);
	strncat(dtg->_path, "/", PATH_MAX);

	return( !chmc_add_dir(dtg->user, dtg->_path)
	        ? chmcerr_code()
	        : CHMC_NOERR );
}
                           ...
                           struct dir_tree_local *dtl )
{
	...
	snprintf(dtg->_path, PATH_MAX, dtg->prefix);
	path_strip_slash(dtg->_path);
	prefixlen = strlen(dtg->_path);

	strncat(dtg->_path, dtg->path, PATH_MAX);
	strncat(dtg->_path, dtl->entry->d_name, PATH_MAX);

	return( chmc_add_file(dtg->user, dtg->_path, prefixlen, 1, NULL,
	                      dtl->statbuf.st_size)
	        ? chmcerr_code()
	        : CHMC_NOERR );
}
